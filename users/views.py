...
class Verification(View):
    ...
    def post(self, request):
        ...
        if verified_obj.verify_otp(user_input):
            messages.success(
                request, f"Account created for {verified_obj.username}")
            return redirect("home-quizzes")
        messages.warning(
            self.request, "Verification Failed! LogIn to try again!")
        return redirect("login")
...
class LoginUser(LoginView):
    ...
    def form_valid(self, form):
        ...
        if self.request.user.is_verified:
            ...
            return redirect("home-quizzes")
        messages.warning(
            self.request, "You are not a verified user. ACCESS denied!")
        return redirect("verification")
